...
class LocalAdapterMixin(LogMixin):
    ...
    @staticmethod
    def __merge_local_csv(dir_prefix, csv_files: list, sep):
    ...
    def __merge_local_csv_parallel(cls, abs_output_path, merge_file_names, sep):
        ...
        for i in range(0, len(merge_file_names), cls.MERGE_CHUNK_SIZE):
            p = cls.executor.submit(cls.__merge_local_csv,
    ...
    @classmethod
    def __merge_local_file(cls, abs_output_path, csv_files, sep):

        if not cls.MERGE_CHUNK_SIZE or cls.MERGE_CHUNK_SIZE == 1:
            return os.path.join(abs_output_path,
                                cls.__merge_local_csv(dir_prefix=abs_output_path, csv_files=csv_files, sep=sep))
        else:
    ...
    def _get_local_sampled_file(cls, output_path, sep):
        ...
        if len(csv_files) > 1:
            cls.log.info("开始合并抽样文件", csv_files)
            csv_file = cls.__merge_local_file(abs_dir, csv_files, sep)
        else:
            csv_file = csv_files[0]
        cls.log.info(f"实际抽样文件为: {csv_file}")
...
class S3OutputAdapterMixin(LogMixin):
    ...
    def _get_s3_sampled_file(cls, output_path, sep, spark):
        ...
        if len(csv_files) > 1:
            cls.log.info(f"生成多个csv文件，s3暂不支持自动合并: {csv_files}", )
            return output_path
        else:
            csv_file = csv_files[0]
        cls.log.info(f"实际抽样文件为: {csv_file}")
